...
# BENCHMARK TOGGLES
# Set to [0] to disable AITER by default (reduces noise). 
# Add 1 to list to re-enable experimental kernels: [0, 1]
AITER_MODES = [0] 
...
def run_throughput(model, tp_size, aiter_mode):
    if tp_size not in MODEL_TABLE[model]["valid_tp"]: return
    
    model_safe = model.replace("/", "_")
    output_file = RESULTS_DIR / f"{model_safe}_tp{tp_size}_aiter{aiter_mode}_throughput.json"
    
    if output_file.exists():
        log(f"SKIP Throughput {model} (TP={tp_size}, AITER={aiter_mode})")
        return
    ...
    log(f"START Throughput {model} (TP={tp_size}, AITER={aiter_mode}) [Batch: {batch_tokens}]...")
    kill_vllm()
    nuke_vllm_cache()
    ...
    env = os.environ.copy()
    env["VLLM_ROCM_USE_AITER"] = str(aiter_mode)
    ...
    try: 
        ...
        log(f"ERROR: Throughput failed {model} (AITER={aiter_mode})")

def run_latency(model, tp_size, aiter_mode):
    if tp_size not in MODEL_TABLE[model]["valid_tp"]: return
    model_safe = model.replace("/", "_")

    if all((RESULTS_DIR / f"{model_safe}_tp{tp_size}_aiter{aiter_mode}_qps{q}_latency.json").exists() for q in QPS_SWEEP):
        return

    dataset_path = get_dataset()
    log(f"START Server {model} (TP={tp_size}, AITER={aiter_mode})...")
    kill_vllm()
    nuke_vllm_cache()

    srv_log = open(RESULTS_DIR / f"{model_safe}_tp{tp_size}_aiter{aiter_mode}_server.log", "w")
    ...
    try:
        ...
        for qps in QPS_SWEEP:
            out_file = RESULTS_DIR / f"{model_safe}_tp{tp_size}_aiter{aiter_mode}_qps{qps}_latency.json"
            if out_file.exists(): continue
            
            log(f"BENCH QPS={qps} (AITER={aiter_mode})...")
            bench_cmd = [
                "vllm", "bench", "serve",
                "--model", model,
...
def print_summary(tps):
    print(f"\n{'MODEL':<40} | {'TP':<2} | {'AITER':<5} | {'TOK/S':<8} | {'QPS':<4} | {'TTFT':<6} | {'TPOT':<6}")
    print("-" * 115)
    ...
    for m in MODELS_TO_RUN:
        ...
        for tp in tps:
            ...
            for aiter in AITER_MODES:
                try: 
                    tdata = json.loads((RESULTS_DIR / f"{msafe}_tp{tp}_aiter{aiter}_throughput.json").read_text())
                ...
                for q in QPS_SWEEP:
                    try:
                        ldata = json.loads((RESULTS_DIR / f"{msafe}_tp{tp}_aiter{aiter}_qps{q}_latency.json").read_text())
                    ...
                    aiter_str = "ON" if aiter == 1 else "OFF"
                    
                    print(f"{name_cell:<40} | {tp:<2} | {aiter_str:<5} | {tok_s:<8} | {q:<4} | {ttft:<6} | {tpot:<6}")
                    first_row = False
            print("-" * 115)
...
if __name__ == "__main__":
    ...
    for tp in args.tp:
        for m in MODELS_TO_RUN:
            for aiter in AITER_MODES:
                run_throughput(m, tp, aiter)
                run_latency(m, tp, aiter)
    print_summary(args.tp)